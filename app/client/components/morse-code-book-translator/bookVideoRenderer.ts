...
export type ResolvedBookVideoBackgroundStyle =
  | "warm-morsewords"
  | "dark-morsewords";

export type BookVideoFrameSize = {
  width: number;
  height: number;
};

type BookVideoTimedEvent = {
  type: "mark" | "gap";
  startMs: number;
  endMs: number;
  symbol?: "." | "-";
...
export type BookVideoTimeline = {
  events: BookVideoTimedEvent[];
  morse: string;
  durationMs: number;
...
type RenderFrameOptions = {
  ...
  frame: BookVideoFrameSize;
  settings: BookVideoSettings;
  timeline: BookVideoTimeline;
...
export function getBookVideoFrameSize(
  resolution: BookVideoSettings["resolution"],
): BookVideoFrameSize {
...
export function getBookVideoFrameRate() {
  return FRAME_RATE;
}
...
export function buildBookVideoTimeline(
  ...
  const timedEvents: BookVideoTimedEvent[] = [];
  let cursorMs = 0;
...
function createBookVideoAudioTrack({
  exportSettings,
  signal,
  timeline,
}: {
  ...
  const context = new AudioContextCtor({ sampleRate: exportSettings.sampleRate });
...
function scheduleBookVideoAudio({
  context,
  destination,
  exportSettings,
  startAtSeconds,
  ...
  const amplitude = Math.max(0, Math.min(1, exportSettings.volume)) * 0.32;
  const attackSeconds = defaultAttackMs(exportSettings.tonePreset) / 1000;
  const releaseSeconds = defaultReleaseMs(exportSettings.tonePreset) / 1000;
...
function getFramePalette(resolvedBackgroundStyle: ResolvedBookVideoBackgroundStyle) {
...
function isMarkActive(timeline: BookVideoTimeline, elapsedMs: number) {
...
function drawDot(
  ...
  accent: string,
  muted: string,
  intensity: BookVideoSettings["intensity"],
) {
...
function drawAnimatedMorseText(
  ...
  ctx.fillText("Morse signal", frame.width / 2, frame.height / 2 - frame.height * 0.16);
  ctx.textAlign = "left";
}
...
function intensityAlpha(intensity: BookVideoSettings["intensity"]) {
