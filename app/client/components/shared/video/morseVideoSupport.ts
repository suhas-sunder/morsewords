...
export function detectMorseVideoSupport(): MorseVideoSupport {
  ...
  const preferredSupport = getPreferredMorseVideoFormatSupportFromList(formats);
  if (!preferredSupport.supported) {
    return unsupported(
      "This browser does not report a supported WebM or MP4 recording format.",
    );
  }
  ...
  return {
    supported: true,
    mimeType: preferredSupport.mimeType,
    extension: preferredSupport.extension,
    reason: `${preferredSupport.label} video export is available in this browser.`,
    audioTrackSupported,
...
export function describeMorseVideoFormat(support: MorseVideoSupport | null) {
  if (!support) return "Checking video support";
  if (!support.supported) return "Video unavailable";
  return getPreferredMorseVideoFormatSupport(support).label;
}

export function getPreferredMorseVideoFormat(
  support: MorseVideoSupport | null,
): MorseVideoFormat {
  return getPreferredMorseVideoFormatSupport(support).format;
}

export function getPreferredMorseVideoFormatSupport(
  support: MorseVideoSupport | null,
): MorseVideoFormatSupport {
  if (!support) {
    return getMorseVideoFormatSupport(null, "webm");
  }
  return getPreferredMorseVideoFormatSupportFromList(support.formats);
}
...
function getPreferredMorseVideoFormatSupportFromList(
  formats: MorseVideoFormatSupport[],
): MorseVideoFormatSupport {
  const mp4Support = getMorseVideoFormatSupportFromList(formats, "mp4");
  if (mp4Support.supported) return mp4Support;
  return getMorseVideoFormatSupportFromList(formats, "webm");
}
