...
const MORSE_VIDEO_MIME_CANDIDATES = [
...
export function detectMorseVideoSupport(): MorseVideoSupport {
  ...
  const mimeType = getSupportedMorseVideoMimeType();
  if (!mimeType) {
...
export function getSupportedMorseVideoMimeType() {
  if (typeof MediaRecorder === "undefined") return "";
  for (const mimeType of MORSE_VIDEO_MIME_CANDIDATES) {
    if (
