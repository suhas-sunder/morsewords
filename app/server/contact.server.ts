...
export async function submitContactForm(request: Request): Promise<ContactResult> {
  ...
  if (name.length < 2 || name.length > 100) {
    return { ok: false, status: 400, message: "Enter a name between 2 and 100 characters." };
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254) {
    return { ok: false, status: 400, message: "Enter a valid email address." };
  }
  if (subject.length < 3 || subject.length > 160) {
    return { ok: false, status: 400, message: "Enter a subject between 3 and 160 characters." };
  }
  if (message.length < 10 || message.length > 4_000) {
    return { ok: false, status: 400, message: "Enter a message between 10 and 4,000 characters." };
  }
  if (!CONTACT_CATEGORIES.includes(category as ContactCategory)) {
    return { ok: false, status: 400, message: "Choose one of the listed contact categories." };
  }
