...
export function getAllowedDirections({
 ...
 const base =
 difficulty ==="easy"? [DIRECTIONS.east, DIRECTIONS.south]
 : difficulty ==="standard"? [
 DIRECTIONS.east,
 DIRECTIONS.south,
 ...
 const reverse =
 difficulty ==="easy"? [DIRECTIONS.west, DIRECTIONS.north]
...
export default function MorseCodeWordSearchBuilder() {
 ...
 const [instructions, setInstructions] = React.useState("Translate each Morse clue into a word, then find that word in the letter grid.",
 );
 ...
 instructions:
 instructions.trim().slice(0, MAX_INSTRUCTIONS_LENGTH) ||"Translate each Morse clue into a word, then find that word in the letter grid.",
 size,
 difficulty,
 allowBackwards,
 ...
 kind:"ok",
 message:
 mode ==="student"?"Question sheet print view opened.": mode ==="answerKey"?"Answer sheet print view opened.":"Question and answer print view opened.",
 }
 : {
 kind:"error",
 ...
 const shareText ="Create a printable Morse code word search with Morse clues, answer keys, and classroom-ready QR branding.";
 ...
 parsed.overflowCount === 1 ?"word was":"words were"} left out.`
 :"",
 parsed.duplicateCount
 ? `${parsed.duplicateCount} duplicate ${
 parsed.duplicateCount === 1 ?"word was":"words were"} ignored.`
 :"",
 ...
 className="mt-2 min-h-48 w-full rounded-xl bg-white p-4 font-mono text-base outline-none focus:ring-2 focus:ring-sky-100" spellCheck={false}
 />
 </label>
 ...
 className="mt-2 min-h-11 w-full rounded-xl bg-white px-3 font-semibold outline-none focus:ring-2 focus:ring-sky-100"/>
 </label>
 ...
 className="mt-2 min-h-24 w-full rounded-xl bg-white p-3 text-sm outline-none focus:ring-2 focus:ring-sky-100"/>
 </label>
 <p className="mt-1 text-xs font-semibold text-slate-500">
 ...
 <CheckToggle
 label="Name line" checked={includeStudentNameLine}
 onChange={setIncludeStudentNameLine}
 />
 <CheckToggle
 label="Date line" checked={includeDateLine}
 ...
 (difficulty === option
 ?"bg-slate-950 text-sky-100 hover:bg-slate-800 hover:text-white":"bg-white text-slate-900 hover:bg-sky-50")
 }
 >
 {option}
 </button>
 ...
 <CheckToggle
 label="Allow backwards words" checked={allowBackwards}
 ...
 <CheckToggle
 label="Show plain answers on student copy" checked={showPlainAnswersOnStudentCopy}
 ...
 <CheckToggle
 label="Include MorseWords branding" checked={includeBranding}
 ...
 {notices.length ? (
 <div className="rounded-xl bg-[#fffdf8] p-3 text-sm leading-relaxed text-slate-700">
 ...
 <ToolHowItWorks
 eyebrow="Puzzle spec" title="How this Morse code word search works" description="This builder makes a Morse-specific puzzle: clues are Morse code, the grid is alphabet letters, and students solve it by translating each clue before searching the grid." referenceLabel="Student task" referenceValue="-- --- .-. ... ." referenceText="Translate the clue first, then find MORSE in the letter grid." chips={[
...
function WordSearchPreview({
 ...
 <article className="mx-auto max-w-3xl rounded-2xl bg-white p-5 text-slate-900 sm:p-7">
 ...
 <img
 src={qrCodeDataUrl}
 alt="QR code to MorseWords" className="h-16 w-16 rounded-xl bg-white p-1"/>
 ) : null}
 </div>

 {(settings.includeStudentNameLine || settings.includeDateLine) ? (
 <div className="mt-4 flex flex-wrap justify-between gap-3 pb-4 text-sm font-semibold text-slate-500">
 ...
 <section className="mt-5 rounded-xl bg-[#fffdf8] p-4">
 ...
 <div
 className="mt-5 grid gap-1" style={{ gridTemplateColumns: `repeat(${settings.size}, minmax(0, 1fr))` }}
 >
 ...
 <span
 key={key}
 className={"flex aspect-square items-center justify-center rounded font-mono text-xs font-black sm:text-sm"+
 (isAnswer
 ?"bg-sky-100 text-sky-950":"bg-white text-slate-950")
 }
 >
 {letter}
 </span>
 ...
 <section className="mt-5 rounded-xl bg-[#f7fbff] p-4">
 ...
 {settings.includeBranding ? (
 <footer className="mt-5 flex flex-wrap items-center justify-between gap-3 pt-4 text-sm text-slate-500">
 <div>
...
function ToolButton({
 ...
 (primary
 ?"bg-slate-950 text-sky-100 hover:bg-slate-800 hover:text-white":"bg-slate-100 text-slate-900 hover:bg-slate-200")
 }
 >
 {icon}
 {children}
...
function StatusNotice({
 ...
 (kind ==="error"?"bg-[#fffdf8] text-slate-700": kind ==="ok"?"bg-sky-50 text-sky-950":"bg-white text-slate-700")
 }
 >
 {kind ==="ok"? (
