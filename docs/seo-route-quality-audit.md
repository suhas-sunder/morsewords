...
| `/morse-code-chart`, `/morse-code-alphabet`, `/morse-code-numbers`, `/morse-code-punctuation`, `/morse-code-printable-chart` | Reference hubs | Index | Interactive and focused references plus a dedicated ready-made chart library | Medium | Distinct metadata and page purposes | Separated ready-made charts from worksheet generation | Keep chart copy focused on downloadable references |
| `/morse-code-printable-worksheets`, `/morse-code-printable-pages` | Printable tools | Index | Custom classroom worksheets versus custom text and processed-book printables | Low | Separate titles, H1s, descriptions, canonicals, and workflows | Worksheet builder moved to its own canonical route | Keep exercise generation distinct from long-text printables |
...
| Chart/reference | `/morse-code-chart`, `/morse-code-alphabet`, `/morse-code-numbers`, `/morse-code-punctuation`, `/morse-code-printable-chart` | Medium | Rebuilt the printable-chart route as the 39-chart download library | Keep interactive lookup pages distinct from ready-made downloads |
| Printable generation | `/morse-code-printable-worksheets`, `/morse-code-printable-pages` | Low | Separated classroom worksheets from custom text and processed-book printables | Preserve the distinct workflows and cross-link by user intent |
