...
const DRY_RUN_ROOT = path.join(AUDIT_REPORT_ROOT, "pilot-dry-run-2");
...
const MAIN_JSON_PATH = path.join(DRY_RUN_ROOT, "pilot-dry-run-2.json");
const MAIN_MARKDOWN_PATH = path.join(DRY_RUN_ROOT, "pilot-dry-run-2.md");
...
const PILOT_SELECTION_REASONS: Record<(typeof PILOT_BATCH)[number], string> = {
  "anne-of-green-gables":
    "Medium-risk book with high-confidence boundaries, an isolated TOC, an existing generated output for comparison, and a feasible preview source.",
  "pointed-roofs":
    "Medium-risk book with high-confidence start/end boundaries and a useful transcriber/printer-note edge case to verify end-matter handling.",
  "the-lost-world":
    "Medium-risk prose novel with high-confidence boundaries, isolated TOC handling, no existing generated output, and a feasible preview source.",
  "the-red-thumb-mark":
    "Medium-risk mystery with a real preface to preserve, high-confidence boundaries, and manageable TOC cleanup.",
  "violet-fairy-book":
    "Medium-risk story collection with a real preface and many sections, selected to check collection-style section splitting without severe artifact risk.",
  "jack-and-jill":
    "Medium-risk generated-output comparison case with high-confidence boundaries and isolated TOC cleanup.",
  "the-wonderful-wizard-of-oz":
    "Medium-risk generated-output comparison case with image placeholder cleanup and high-confidence readable boundaries.",
  "the-legend-of-sleepy-hollow":
    "Medium-risk short work with a medium-confidence opening epigraph, selected as a controlled boundary review case.",
  "four-day-planet":
    "Medium-risk science-fiction novel with dedication/TOC/transcriber-note cleanup signals and high-confidence boundaries.",
  "room-13":
    "Medium-risk novel with high-confidence boundaries and an end-note/correction edge case that should remain reviewable before writing.",
  "the-octopus-a-story-of-california":
    "Medium-risk long novel with clear book/chapter structure, selected to test large but structured processing.",
  "the-prince-and-the-pauper":
    "Medium-risk novel with high-confidence boundaries plus footnote/reference cleanup that should be reviewed before writing.",
  triplanetary:
    "Medium-risk science-fiction novel with high-confidence boundaries and illustration placeholder cleanup.",
  "the-call-of-the-wild":
    "Medium-risk generated-output comparison case with high-confidence boundaries and a feasible first-hour preview source.",
};

const EXCLUDED_FROM_BATCH_2 = [
...
function formattedContext(
  ...
  return lines
    .slice(Math.max(0, startLineIndex), Math.min(lines.length, endLineIndex + 1))
    .map((line) => `L${line.lineNumber}: ${textPreview(line.text, 180)}`);
}
...
function main(): void {
  ...
  if (PILOT_BATCH.length < 12 || PILOT_BATCH.length > 15) {
    throw new Error(`Pilot dry-run 2 must inspect 12-15 books; found ${PILOT_BATCH.length}.`);
  }
  const excluded = new Set<string>(EXCLUDED_FROM_BATCH_2);
  const forbiddenSelections = PILOT_BATCH.filter((slug) => excluded.has(slug));
  if (forbiddenSelections.length > 0) {
    throw new Error(`Pilot dry-run 2 includes excluded slugs: ${forbiddenSelections.join(", ")}`);
  }
  ...
  console.log("Pilot dry run 2 complete.");
