...
const dryRunRoot = path.join(auditRoot, "pilot-dry-run-4");
const writeReportRoot = path.join(auditRoot, "pilot-write-4");
...
function buildManifest(
  ...
  return {
    ...
    warnings: [
      "Generated by controlled pilot write pass 4; review before scaling to larger batches or Cloudflare export.",
...
function processAcceptedBook(dryRun: DryRunBook): BookReport {
  ...
  return {
    ...
    acceptedFromDryRunReason: "Dry-run batch 4 classified this book as already acceptable; generated output and preview were inspected but not rewritten.",
    noFilesChanged: true,
  };
}
...
function processCorrectionBook(
  ...
  warnings.push(
    `Structure detector observed ${structureAnalysis.detectedStructuralConvention}; source-backed write-4 plan used ${plan.structuralConvention}.`,
  );
  ...
  const rightsReport: BookRightsReport = {
    ...
    reasoning_summary:
      "Controlled pilot write pass 4 used source-backed boundaries from dry-run batch 4 review and did not modify raw sources or Cloudflare exports.",
  };
...
function assertDryRunShape(report: DryRunReport) {
  if (report.reportName !== "pilot-dry-run-4") {
  ...
  if (JSON.stringify(selected) !== JSON.stringify(expectedSelected)) {
    throw new Error("Dry-run selected books do not match the requested batch-4 list.");
  }
  ...
  if (
    report.totals.selectedBooks !== 25 ||
    report.totals.alreadyAcceptable !== 11 ||
    report.totals.needsCorrectionBeforeAcceptance !== 14 ||
    report.totals.manualReview !== 0 ||
    report.totals.blocked !== 0
  ) {
    throw new Error("Dry-run totals are not the expected 25/11/14/0/0 split.");
  }
}
...
function writeReport(results: BookReport[]) {
  ...
  const jsonReport = {
    ...
    mode: "controlled write/correction batch 4",
    inputReports: {
      pilotDryRun4Json: statusPath(dryRunReportPath),
      pilotDryRun4Markdown: statusPath(path.join(dryRunRoot, "pilot-dry-run-4.md")),
      pilotDryRun4Books: statusPath(path.join(dryRunRoot, "books")),
    },
    protectedPaths: {
    ...
    confirmations: {
      dryRunBatch4MergedToMainFirst: true,
      processedOnlySelectedBatch4Books: true,
  ...
  writeJson(path.join(writeReportRoot, "pilot-write-4.json"), jsonReport);
  ...
  const previewFiles = corrected
    ...
    .map((filePath) => `- ${filePath}`)
    .join("\n");
  const markdown = `# Pilot Write 4 Report
...
`;
  writeText(path.join(writeReportRoot, "pilot-write-4.md"), markdown);
}
...
function main() {
  ...
  const correctionPlans = makeCorrectionPlans();
  ...
  writeReport(results);
  console.log(
    `Pilot write 4 complete: ${results.filter((result) => result.finalAction === "accepted without rewrite").length} accepted without rewrite, ${results.filter((result) => result.finalAction === "corrected").length} corrected, ${results.filter((result) => result.finalAction === "skipped").length} skipped.`,
  );
}
