...
const dryRunRoot = path.join(auditRoot, "pilot-dry-run-10");
const writeRoot = path.join(auditRoot, "pilot-write-10");
const verificationRoot = path.join(auditRoot, "pilot-write-10-verification");
...
const PLAYWRIGHT_TRIAGE: PlaywrightTriage = {
  ...
  priorFailureSurface:
    "general approved external-authority Gutenberg book public page UI, not a batch-10 generated book page",
  writeBranchFullSuite:
    "2026-06-19 verification rerun on morsewords-book-processing-pilot-write-10-jun-2026: 36 passed with --reporter=line",
  mainFullSuite:
    "2026-06-19 comparison rerun on latest main after git fetch/pull and typecheck: 36 passed with --reporter=line",
...
const AUDIT_SIDE_EFFECT_HANDLING: AuditSideEffectHandling = {
  ...
  handling:
    "books:title-start-default-audit is known to reapply 12 unrelated older generated-book corrections; validation side effects are restored unless they are explicit batch-10 corrections.",
};
...
function verifyProcessedBook(
  ...
  return {
    ...
    dryRunReportInspected: repoPath(path.join(dryRunRoot, "pilot-dry-run-10.json")),
    dryRunPerBookReportsInspected: [repoPath(dryRunBookJsonPath), repoPath(dryRunBookMdPath)],
    writeReportInspected: repoPath(path.join(writeRoot, "pilot-write-10.json")),
    generatedTitleVerdict,
    generatedAuthorVerdict,
...
function writeMarkdownReport(report: {
  ...
  writeText(
    path.join(verificationRoot, "pilot-write-10-verification.md"),
    `${lines.join("\n").trimEnd()}\n`,
  );
}

function main() {
  const dryRunJsonPath = path.join(dryRunRoot, "pilot-dry-run-10.json");
  const dryRunMdPath = path.join(dryRunRoot, "pilot-dry-run-10.md");
  const writeJsonPath = path.join(writeRoot, "pilot-write-10.json");
  const writeMdPath = path.join(writeRoot, "pilot-write-10.md");
  ...
  if (dryRun.reportName !== "pilot-dry-run-10" || write.reportName !== "pilot-write-10") {
    throw new Error("Missing or incorrect batch-10 dry-run/write reports.");
  }
  if (JSON.stringify(dryRun.selectedBooks) !== JSON.stringify([...SELECTED_BATCH])) {
    throw new Error("Dry-run selectedBooks do not match the expected batch-10 list.");
  }
  if (JSON.stringify(write.selectedBooks) !== JSON.stringify([...SELECTED_BATCH])) {
    throw new Error("Write selectedBooks do not match the expected batch-10 list.");
  }
  if (
    ...
    throw new Error("Write report is missing or incomplete for batch 10.");
  }
  ...
  const report = {
    schemaVersion: 1,
    reportName: "pilot-write-10-verification",
    generatedAt,
  ...
  writeJson(path.join(verificationRoot, "pilot-write-10-verification.json"), report);
  writeMarkdownReport({
    generatedAt,
  ...
  console.log(
    `Pilot write 10 verification complete: ${pass} pass, ${warnAccepted} warn accepted, ${fail} fail; ${correctionsApplied} corrections documented.`,
  );
