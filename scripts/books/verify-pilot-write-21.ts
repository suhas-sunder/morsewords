...
const dryRunRoot = path.join(auditRoot, "pilot-dry-run-21");
const writeRoot = path.join(auditRoot, "pilot-write-21");
const verificationRoot = path.join(auditRoot, "pilot-write-21-verification");
...
function roleEvidencePass(dryBook: JsonRecord, writeBook: JsonRecord, rawText: string, rights: JsonRecord): boolean {
  ...
  return expectedAuthors.length === 1 &&
    ["H. G. Wells", "Herbert George Wells"].includes(String(expectedAuthors[0])) &&
...
function verifyBook(dryBook: JsonRecord, writeBook: JsonRecord) {
  ...
  return {
    ...
    authorVerdict: verdict(checks.metadata ? "pass" : "fail", checks.metadata
      ? `${generatedAuthors.join(", ")} is source-backed as author, no Unknown Author appears, and no parent collection metadata replaces the individual story metadata.`
...
function writeMarkdown(report: JsonRecord) {
  const lines = [
    "# Pilot write batch 21 verification", "", `Generated: ${report.generatedAt}`, "", "## Summary", "",
    ...
    ...report.books.flatMap((book: JsonRecord) => [
      ...
      `- Author metadata: ${book.authorVerdict.status} - ${book.authorVerdict.summary}`,
  ...
  writeText(path.join(verificationRoot, "pilot-write-21-verification.md"), `${lines.join("\n").trimEnd()}\n`);
}

function main() {
  const requiredReports = [
    path.join(dryRunRoot, "pilot-dry-run-21.json"),
    path.join(dryRunRoot, "pilot-dry-run-21.md"),
    path.join(writeRoot, "pilot-write-21.json"),
    path.join(writeRoot, "pilot-write-21.md"),
  ...
  const dry = readJson(path.join(dryRunRoot, "pilot-dry-run-21.json"));
  const write = readJson(path.join(writeRoot, "pilot-write-21.json"));
  const batch12 = readJson(path.join(auditRoot, "batch-12-prose-restoration/batch-12-prose-restoration.json"));
  if (dry.reportName !== "pilot-dry-run-21" || write.reportName !== "pilot-write-21") throw new Error("Report identity mismatch.");
  ...
  const report = {
    schemaVersion: 1,
    reportName: "pilot-write-21-verification",
    generatedAt: new Date().toISOString(),
    branch: "morsewords-book-processing-pilot-write-21-jun-2026",
    sourceWriteCommit: "187621dce13f66a51a41fdb846ce091822b25082",
    scope: "post-write QA/review of the exact 20 pilot write batch 21 books; no additional books processed",
    ...
    sharedWriteScriptScopeFinding: {
      files: ["scripts/books/pilot-book-processing-write-12.ts", "scripts/books/pilot-book-processing-write-21.ts"],
    ...
    specialFocus: {
      exactSpelling: "Aepyornis, Avu, Genus Novo, Hammerpond Park, Harringay, Taxidermist, and The Country of the Blind match source-audited titles exactly.",
      creatorRoles: "19/19 sources naming H. G. Wells preserve that exact author metadata; The Crystal Egg preserves its source-backed Herbert George Wells metadata; no batch-21 title uses Unknown Author.",
    ...
    validationResults: {
      ...
      pilotWrite21: process.env.MORSEWORDS_VERIFY_WRITE21 ?? "pending",
    ...
    browserAndPlaywright: {
      inAppBrowserNote: process.env.MORSEWORDS_VERIFY_BROWSER ?? "pending",
  ...
  writeJson(path.join(verificationRoot, "pilot-write-21-verification.json"), report);
  writeMarkdown(report);
  console.log(`Pilot write 21 verification: ${pass} pass, ${warnAccepted} warn accepted, ${fail} fail; raw/generated ${rawExact}/${books.length} exact.`);
