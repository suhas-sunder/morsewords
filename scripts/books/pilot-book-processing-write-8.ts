...
const dryRunRoot = path.join(
  repoRoot,
  "app/client/assets/books/audit-reports/pilot-dry-run-8",
);
const writeReportRoot = path.join(
  repoRoot,
  "app/client/assets/books/audit-reports/pilot-write-8",
);
const dryRunReportPath = path.join(dryRunRoot, "pilot-dry-run-8.json");
...
function makeMetadata(
  ...
  return {
    ...
    source: {
      ...
      rightsReviewed: true,
      rightsNotes:
        "Pilot write pass 8 processed this audited Project Gutenberg source for review-gated MorseWords book output.",
    },
    cover: {
      src: null,
      placeholder: true,
...
function buildManifest(
  ...
  return {
    ...
    source: {
      ...
      rightsNotes:
        "Pilot write pass 8 processed this source from the audited Project Gutenberg text. Review generated output before any Cloudflare export.",
    },
    cover: {
      src: null,
      placeholder: true,
    ...
    warnings: [
      "Generated by controlled pilot write pass 8; review before scaling to larger batches or Cloudflare export.",
...
function buildRightsReport(
  ...
  return {
    ...
    reasoning_summary:
      "Controlled pilot write pass 8 used audited Project Gutenberg public-domain source text after dry-run review. Generated output remains review-gated before any Cloudflare export.",
    evidence_snippets: [
...
function duplicateNearDuplicateCheck(dryRun: DryRunBook): {
  ...
  if (duplicate.slug === dryRun.slug) {
    return {
      blocking: false,
      message: `passed: ${duplicate.slug} is the selected batch-8 slug and may be regenerated by this targeted command`,
    };
  }

  return {
    blocking: true,
    message: `blocked: existing generated slug ${duplicate.slug} already has title "${duplicate.title}" and author ${duplicate.author.join(", ")}; dry-run 8 did not document a distinct-version reason for creating ${dryRun.slug}`,
  };
}
...
Processed by pilot write pass 8.
...
function writeMarkdownReport(report: {
  ...
  writeText(path.join(writeReportRoot, "pilot-write-8.md"), `${lines.join("\n").trimEnd()}\n`);
}

function main() {
  const dryRun = readJson<DryRunReport>(dryRunReportPath);
  if (dryRun.reportName !== "pilot-dry-run-8") {
  ...
  writeJson(path.join(writeReportRoot, "pilot-write-8.json"), jsonReport);
  writeMarkdownReport({
    generatedAt,
  ...
  console.log(
    `Pilot write 8 complete: ${processed.length} first-time processed, ${skipped.length} skipped, ${dryRun.unresolvedSourceGeneratedBooksLeftUntouched.length} unresolved-source generated books untouched.`,
  );
}
