...
const reportRoot = path.join(auditRoot, "pilot-dry-run-5");
...
const WRITE_4_VERIFICATION_JSON_PATH = path.join(
  auditRoot,
  "pilot-write-4-verification/pilot-write-4-verification.json",
);
const LIBRARY_MANIFEST_PATH = path.join(generatedRoot, "library-manifest.json");
const MAIN_JSON_PATH = path.join(reportRoot, "pilot-dry-run-5.json");
const MAIN_MARKDOWN_PATH = path.join(reportRoot, "pilot-dry-run-5.md");
...
const UNSELECTED_GENERATED_CANDIDATES_WITHOUT_RESOLVED_RAW_SOURCE = [
  "a-princess-of-mars",
  "doctor-dolittle",
  "heidi",
  "jabberwocky",
...
function fallbackPriorPassBook(
  ...
  return {
    ...
    pass2RiskReasons: [
      "Book is not present in the older pass-2 audit report; batch-5 dry run used local raw source, generated output, preview asset, startup audit, and structure detector directly.",
    ],
    firstHourPreviewCanBeSafelyDerivedLater: true,
    recommendedNextAction: "batch-5 dry-run review",
  };
}
...
function mainMarkdown(report: DryRunReport) {
  ...
  return [
    ...
    "",
    report.selectionRules.unselectedGeneratedCandidatesWithoutResolvedRawSource
      .map((slug) => `- ${slug}`)
      .join("\n"),
    "",
    "## Totals",
...
function assertInputs() {
  for (const requiredPath of [
    ...
    STRUCTURE_JSON_PATH,
    STARTUP_JSON_PATH,
    WRITE_4_VERIFICATION_JSON_PATH,
  ...
  if (forbidden.length > 0) {
    throw new Error(`Pilot dry-run 5 includes forbidden slugs: ${forbidden.join(", ")}`);
  }
}
...
function main() {
  ...
  const books = PILOT_BATCH.map((slug) => {
    ...
    const result = inspectBook(
      slug,
      libraryBook,
      pass2Book ?? fallbackPriorPassBook(slug, libraryBook),
  ...
  const report: DryRunReport = {
    ...
    branch: "morsewords-book-processing-pilot-dry-run-5-jun-2026",
    baseMainCommit,
    mode: "dry-run/report-only",
    ...
    selectionRules: {
      ...
      unselectedGeneratedCandidatesWithoutResolvedRawSource: [
        ...UNSELECTED_GENERATED_CANDIDATES_WITHOUT_RESOLVED_RAW_SOURCE,
      ],
      note: "Selected from generated books with valid startup previews, excluding accepted/corrected and known manual/blocked/suspicious slugs. Exactly 20 remaining generated-but-unaccepted books had raw source files resolvable under app/client/assets/temp-books, so the batch uses those 20 and does not force the remaining unresolved-source candidates. The set mixes chapter novels, story collections, single-story fairy tales, and one poem-like/short-form group where the local raw source and generated output could be inspected. No nonfiction/essay works remained in the eligible generated pool with resolvable raw source.",
    },
    inputReports: [
      ...
      relativeToRepo(WRITE_4_VERIFICATION_JSON_PATH),
    ],
    protectedPaths: {
  ...
  console.log(`Pilot dry-run 5 books inspected: ${report.totals.selectedBooks}`);
  console.log(
