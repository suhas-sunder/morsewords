...
const dryRunRoot = path.join(
  repoRoot,
  "app/client/assets/books/audit-reports/pilot-dry-run-10",
);
const writeReportRoot = path.join(
  repoRoot,
  "app/client/assets/books/audit-reports/pilot-write-10",
);
const dryRunReportPath = path.join(dryRunRoot, "pilot-dry-run-10.json");
...
const EXPECTED_FINAL_SECTION_COUNTS: Record<string, number> = {
  ...
  kidnapped: 30,
  "oliver-twist": 53,
  "the-benson-murder-case": 25,
  "the-inspector-french-s-greatest-case": 20,
  "murder-in-the-maze": 18,
  "the-house-of-arden-a-story-for-children": 14,
  "the-shadow-over-innsmouth": 5,
  "the-thing-on-the-door-step": 7,
  "at-the-mountains-of-madness": 12,
  "the-remarkable-case-of-davidson-s-eyes": 5,
  "the-haunter-of-the-dark": 1,
  "the-innocence-of-father-brown": 12,
  "astounding-stories-of-super-science": 6,
  "a-story-of-the-stone-age": 5,
  "the-magic-shop": 1,
  "the-man-who-could-work-miracles": 1,
  "the-truth-about-pyecraft": 1,
  filmer: 1,
  "two-in-a-sack": 1,
};
...
> = {
  "the-benson-murder-case": {
    ...
    warning:
      "Write pass adjusted display-title capitalization from the Project Gutenberg Title line after confirming the same source identity.",
  },
  "murder-in-the-maze": {
...
function makeMetadata(
  ...
  return {
    ...
    source: {
      ...
      rightsReviewed: true,
      rightsNotes:
        "Pilot write pass 10 processed this audited Project Gutenberg source for review-gated MorseWords book output.",
    },
    cover: {
      src: null,
      placeholder: true,
...
const SINGLE_STORY_START_PHRASES: Record<string, string> = {
  "the-haunter-of-the-dark": "I have seen the dark universe yawning",
  "the-magic-shop": "I had seen the Magic Shop from afar several times",
  "the-man-who-could-work-miracles": "It is doubtful whether the gift was innate",
  "the-truth-about-pyecraft": "He sits not a dozen yards away",
  filmer: "In truth the mastery of flying was the work of thousands of men",
  "two-in-a-sack": "What a life that poor man led with his wife",
};
...
function buildManifest(
  ...
  return {
    ...
    source: {
      ...
      rightsNotes:
        "Pilot write pass 10 processed this source from the audited Project Gutenberg text. Review generated output before any Cloudflare export.",
    },
    cover: {
      src: null,
      placeholder: true,
    ...
    warnings: [
      "Generated by controlled pilot write pass 10; review before scaling to larger batches or Cloudflare export.",
...
function buildRightsReport(
  ...
  return {
    ...
    reasoning_summary:
      "Controlled pilot write pass 10 used audited Project Gutenberg public-domain source text after dry-run review. Generated output remains review-gated before any Cloudflare export.",
    evidence_snippets: [
...
function duplicateNearDuplicateCheck(dryRun: DryRunBook): {
  ...
  if (duplicate.slug === dryRun.slug) {
    return {
      blocking: false,
      message: `passed: ${duplicate.slug} is the selected batch-10 slug and may be regenerated by this targeted command`,
    };
  }

  return {
    blocking: true,
    message: `blocked: existing generated slug ${duplicate.slug} already has title "${duplicate.title}" and author ${duplicate.author.join(", ")}; dry-run 10 did not document a distinct-version reason for creating ${dryRun.slug}`,
  };
}
...
Processed by pilot write pass 10.
...
function writeMarkdownReport(report: {
  ...
  writeText(path.join(writeReportRoot, "pilot-write-10.md"), `${lines.join("\n").trimEnd()}\n`);
}

function main() {
  const dryRun = readJson<DryRunReport>(dryRunReportPath);
  if (dryRun.reportName !== "pilot-dry-run-10") {
  ...
  const acceptedExclusionClarification = {
    ...
    note:
      "This write pass uses only the exact selected list from pilot-dry-run-10.json. Known duplicate/manual/boundary exclusions and unresolved-source generated books remain untouched.",
  };
  const jsonReport = {
    schemaVersion: 1,
    reportName: "pilot-write-10",
    generatedAt,
    branch: "morsewords-book-processing-pilot-write-10-jun-2026",
  ...
  writeJson(path.join(writeReportRoot, "pilot-write-10.json"), jsonReport);
  writeMarkdownReport({
    generatedAt,
  ...
  console.log(
    `Pilot write 10 complete: ${processed.length} first-time processed, ${skipped.length} skipped, ${dryRun.unresolvedSourceGeneratedBooksLeftUntouched.length} unresolved-source generated books untouched.`,
  );
}
