...
const reportRoot = path.join(auditRoot, "pilot-dry-run-11");
const reportBooksRoot = path.join(reportRoot, "books");
const mainJsonPath = path.join(reportRoot, "pilot-dry-run-11.json");
const mainMarkdownPath = path.join(reportRoot, "pilot-dry-run-11.md");
...
const singleStoryStartPhrases: Record<string, string> = {
  ...
  "the-adventures-of-kintaro-the-golden-boy":
    "Long, long ago there lived in Kyoto",
  "the-bamboo-cutter-and-the-moon-child":
...
function buildReport() {
  ...
  const allowedBranches = new Set([
    "main",
    "morsewords-book-processing-pilot-dry-run-11-jun-2026",
  ]);
  if (!allowedBranches.has(branch)) {
    throw new Error(`Expected dry-run 11 validation branch, got ${branch}`);
  }
  ...
  console.log(`Pilot dry-run 11 selected: ${report.selectedCount}`);
  console.log(
