...
const reportRoot = path.join(auditRoot, "pilot-dry-run-12");
const reportBooksRoot = path.join(reportRoot, "books");
const mainJsonPath = path.join(reportRoot, "pilot-dry-run-12.json");
const mainMarkdownPath = path.join(reportRoot, "pilot-dry-run-12.md");
...
function buildReport() {
  ...
  const allowedBranches = new Set([
    "main",
    "morsewords-book-processing-pilot-dry-run-12-jun-2026",
  ]);
  if (!allowedBranches.has(branch)) {
    throw new Error(`Expected dry-run 12 validation branch, got ${branch}`);
  }
  ...
  console.log(`Pilot dry-run 12 selected: ${report.selectedCount}`);
  console.log(
