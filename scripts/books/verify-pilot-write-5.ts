...
const dryRunMarkdownPath = path.join(dryRunRoot, "pilot-dry-run-5.md");
const writeReportPath = path.join(writeRoot, "pilot-write-5.json");
const writeMarkdownPath = path.join(writeRoot, "pilot-write-5.md");
const verificationJsonPath = path.join(
  verificationRoot,
  "pilot-write-5-verification.json",
);
const verificationMarkdownPath = path.join(
  verificationRoot,
  "pilot-write-5-verification.md",
);
...
const focusedVerificationFixes: Array<{
  slug: string;
...
function mainReadableSections(sections: GeneratedBookSectionJson[]) {
  return sections.filter((section) => isDefaultReadableSection(section));
}
...
function normalizedSectionText(...parts: Array<string | null | undefined>) {
  return parts
...
function sectionNameText(section: GeneratedBookSectionJson) {
  return normalizedSectionText(section.label, section.title);
}

function sectionEvidenceText(section: GeneratedBookSectionJson) {
  return normalizedSectionText(section.label, section.title, section.textPreview);
}

function isDefaultReadableSection(section: GeneratedBookSectionJson) {
  if (nonPlayableKinds.has(section.kind)) return false;
  ...
  if (asideDefaultEvidenceExclusionPattern.test(evidenceText)) return false;
...
function explicitDefaultSections(sections: GeneratedBookSectionJson[]) {
  return sections.filter((section) => section.includeByDefault);
}

function defaultSections(sections: GeneratedBookSectionJson[]) {
  const explicit = explicitDefaultSections(sections);
  if (explicit.length > 0) return explicit;

  const readable = sections.filter((section) => isDefaultReadableSection(section));
...
function verifySectioning(
  ...
  const explicitBodySections = explicitDefaultSections(sections);
  const bodySections = defaultSections(sections);
  if (manifest.stats.includedSectionCount !== explicitBodySections.length) {
    return verdict("fail", "Manifest included-section count does not match defaults.", [
      `Manifest: ${manifest.stats.includedSectionCount}; explicit defaults loaded: ${explicitBodySections.length}.`,
    ]);
  }
  if (bodySections.length === 0) {
    return verdict("fail", "No generated sections are included by default.");
  }
  if (explicitBodySections.length === 0) {
    details.push("No explicit includeByDefault sections; verified runtime/startup fallback selects readable sections.");
  }
      ...
      writeBook.finalAction === "accepted without rewrite"
        ? {
            whyNoRewriteWasNeeded:
              ...
              "Dry-run batch 5 classified this book as already acceptable.",
...
function assertReportShape(dryRunReport: DryRunReport, writeReport: WriteReport) {
  if (dryRunReport.reportName !== "pilot-dry-run-5") {
    throw new Error("Dry-run report is not pilot-dry-run-5.");
  }
  if (writeReport.reportName !== "pilot-write-5") {
  ...
  if (JSON.stringify([...dryRunReport.selectedBooks].sort()) !== JSON.stringify(selected)) {
    throw new Error("Dry-run selected books do not match batch-5.");
  }
  if (JSON.stringify([...writeReport.selectedBooks].sort()) !== JSON.stringify(selected)) {
    throw new Error("Write selected books do not match batch-5.");
  }
  if (dryRunReport.books.length !== 20 || writeReport.books.length !== 20) {
    throw new Error("Expected 20 books in both dry-run and write reports.");
  }
  if (
    dryRunReport.totals.alreadyAcceptable !== 11 ||
    dryRunReport.totals.needsCorrectionBeforeAcceptance !== 9 ||
    writeReport.totals.acceptedWithoutRewrite !== 11 ||
    writeReport.totals.corrected !== 9 ||
    writeReport.totals.skipped !== 0
  ) {
    throw new Error("Batch-5 dry-run/write totals are not the expected 11/9/0 split.");
  }
...
function buildMarkdown(report: ReturnType<typeof buildReport>) {
  const lines: string[] = [];
  lines.push("# Pilot Write 5 Verification");
  lines.push("");
  ...
  lines.push("");
  if (report.focusedCorrectionNotes.length === 0) {
    lines.push("- No verification-time corrections were made.");
    lines.push("");
  } else {
...
function buildReport(books: BookVerification[]) {
  return {
    reportName: "pilot-write-5-verification",
...
function main() {
  ...
  const books = selectedBatch.map((slug) => {
    ...
    if (!dry || !write) throw new Error(`Missing batch-5 book report: ${slug}`);
  ...
  console.log(
    `Pilot Write 5 verification complete: ${report.totals.pass} pass, ${report.totals.warnAccepted} warn accepted, ${report.totals.fail} fail.`,
  );
}
