...
type VerificationBook = {
  ...
  generatedAuthorCompilerCollectorTranslatorVerdict: Verdict;
...
const dryRunRoot = path.join(auditRoot, "pilot-dry-run-12");
const writeRoot = path.join(auditRoot, "pilot-write-12");
const verificationRoot = path.join(auditRoot, "pilot-write-12-verification");
...
const PLAYWRIGHT_TRIAGE: PlaywrightTriage = {
  ...
  priorFailureSurface:
    "general approved external-authority Gutenberg book public page UI, specifically Treasure Island, not a batch-12 generated book page",
  writeBranchFullSuite:
    "2026-06-19 write batch 12 validation reported 36/36 passed; verification rerun on morsewords-book-processing-pilot-write-12-jun-2026 also passed 36/36 with --reporter=line",
  mainFullSuite:
    "Not rerun during batch-12 verification because the write pass already reported 36/36 passed and fullscreen UX work is explicitly deferred.",
  writeBranchNonFullscreenSubset:
    "Not needed during batch-12 verification; no fullscreen failure appeared in the write pass result.",
...
const AUDIT_SIDE_EFFECT_HANDLING: AuditSideEffectHandling = {
  ...
  handling:
    "books:title-start-default-audit is known to reapply 12 unrelated older generated-book corrections; validation side effects are restored unless they are explicit batch-12 corrections.",
};
...
function verifyProcessedBook(
  ...
  const titleMatches =
    ...
    sourceContainsSnippet(rawText, writeBook.supportingSnippets.title || manifest.title, 3);
  ...
  return {
    ...
    dryRunReportInspected: repoPath(path.join(dryRunRoot, "pilot-dry-run-12.json")),
    dryRunPerBookReportsInspected: [repoPath(dryRunBookJsonPath), repoPath(dryRunBookMdPath)],
    writeReportInspected: repoPath(path.join(writeRoot, "pilot-write-12.json")),
    generatedTitleVerdict,
    generatedAuthorVerdict,
    generatedAuthorCompilerCollectorTranslatorVerdict: generatedAuthorVerdict,
...
function writeMarkdownReport(report: {
  ...
  const lines = [
    ...
    ...report.books.flatMap((book) => [
      ...
      `- Generated author/compiler/collector/translator verdict: ${book.generatedAuthorCompilerCollectorTranslatorVerdict.summary}`,
  ...
  writeText(
    path.join(verificationRoot, "pilot-write-12-verification.md"),
    `${lines.join("\n").trimEnd()}\n`,
  );
}

function main() {
  const dryRunJsonPath = path.join(dryRunRoot, "pilot-dry-run-12.json");
  const dryRunMdPath = path.join(dryRunRoot, "pilot-dry-run-12.md");
  const writeJsonPath = path.join(writeRoot, "pilot-write-12.json");
  const writeMdPath = path.join(writeRoot, "pilot-write-12.md");
  ...
  if (dryRun.reportName !== "pilot-dry-run-12" || write.reportName !== "pilot-write-12") {
    throw new Error("Missing or incorrect batch-12 dry-run/write reports.");
  }
  if (JSON.stringify(dryRun.selectedBooks) !== JSON.stringify([...SELECTED_BATCH])) {
    throw new Error("Dry-run selectedBooks do not match the expected batch-12 list.");
  }
  if (JSON.stringify(write.selectedBooks) !== JSON.stringify([...SELECTED_BATCH])) {
    throw new Error("Write selectedBooks do not match the expected batch-12 list.");
  }
  if (
    ...
    throw new Error("Write report is missing or incomplete for batch 12.");
  }
  ...
  const report = {
    schemaVersion: 1,
    reportName: "pilot-write-12-verification",
    generatedAt,
  ...
  writeJson(path.join(verificationRoot, "pilot-write-12-verification.json"), report);
  writeMarkdownReport({
    generatedAt,
  ...
  console.log(
    `Pilot write 12 verification complete: ${pass} pass, ${warnAccepted} warn accepted, ${fail} fail; ${correctionsApplied} corrections documented.`,
  );
