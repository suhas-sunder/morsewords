...
const dryRunRoot = path.join(auditRoot, "pilot-dry-run-19");
const writeRoot = path.join(auditRoot, "pilot-write-19");
const verificationRoot = path.join(auditRoot, "pilot-write-19-verification");
...
const SPECIAL_SPELLINGS = new Map<string, string>([
  ...
  ["the-nine-pea-hens-and-the-golden-apples", "The Nine Pea-Hens and the Golden Apples"],
  ["the-monkey-and-the-jelly-fish", "The Monkey and the Jelly-Fish"],
  ["the-young-man-who-would-have-his-eyes-opened", "The Young Man Who Would Have His Eyes Opened"],
]);
...
function sourceLooksUnsafe(text: string): boolean {
  return /Project Gutenberg|Gutenberg License|START OF (?:THE|THIS) PROJECT GUTENBERG|END OF (?:THE|THIS) PROJECT GUTENBERG|www\.gutenberg|Distributed Proofreading|Transcriber's Notes?|^Author:|^Title:|^Release date:|^Edited by Andrew Lang$|Contact Us|Site Map|All Rights Reserved|(?:^|\n)\s*(?:CONTENTS|TABLE OF CONTENTS)\s*(?:\n|$)/im.test(text);
}
...
function roleEvidencePass(dryBook: JsonRecord, writeBook: JsonRecord, rawText: string, rights: JsonRecord): boolean {
  ...
  return evidence.length > 0 &&
    evidence.every((item) => rawText.includes(String(item.text ?? ""))) &&
    String(dryBook.expectedCreatorRole ?? "").startsWith("editor: Andrew Lang") &&
...
function verifyBook(dryBook: JsonRecord, writeBook: JsonRecord) {
  ...
  return {
    ...
    authorCompilerCollectorTranslatorRetellerEditorVerdict: verdict(checks.metadata ? "pass" : "fail", checks.metadata
      ...
      : "Creator/editor metadata is missing, unknown, inconsistent with source evidence, or loses Andrew Lang's editor role.",
...
function writeMarkdown(report: JsonRecord) {
  const lines = [
    "# Pilot write batch 19 verification", "", `Generated: ${report.generatedAt}`, "", "## Summary", "",
    ...
    `- Andrew Lang role: ${report.specialFocus.andrewLangEditorRole}`,
  ...
  writeText(path.join(verificationRoot, "pilot-write-19-verification.md"), `${lines.join("\n").trimEnd()}\n`);
}

function main() {
  const requiredReports = [
    path.join(dryRunRoot, "pilot-dry-run-19.json"),
    path.join(dryRunRoot, "pilot-dry-run-19.md"),
    path.join(writeRoot, "pilot-write-19.json"),
    path.join(writeRoot, "pilot-write-19.md"),
  ...
  const dry = readJson(path.join(dryRunRoot, "pilot-dry-run-19.json"));
  const write = readJson(path.join(writeRoot, "pilot-write-19.json"));
  const batch12 = readJson(path.join(auditRoot, "batch-12-prose-restoration/batch-12-prose-restoration.json"));
  if (dry.reportName !== "pilot-dry-run-19" || write.reportName !== "pilot-write-19") throw new Error("Report identity mismatch.");
  ...
  const report = {
    schemaVersion: 1,
    reportName: "pilot-write-19-verification",
    generatedAt: new Date().toISOString(),
    branch: "morsewords-book-processing-pilot-write-19-jun-2026",
    sourceWriteCommit: "2624c2c162e334781e23ae60519819f4257561b2",
    scope: "post-write QA/review of the exact 20 pilot write batch 19 books; no additional books processed",
    ...
    write12SharedScriptScopeFinding: {
      files: ["scripts/books/pilot-book-processing-write-12.ts", "scripts/books/pilot-book-processing-write-19.ts"],
    ...
    specialFocus: {
      exactSpelling: "Halfman, Hassebu, Nunda, Pea-Hens, Jelly-Fish, and Eyes Opened match source-audited titles exactly.",
      andrewLangEditorRole: "20/20 rights reports preserve Andrew Lang as editor from visible source evidence; no batch-19 title uses Unknown Author.",
    ...
    validationResults: {
      ...
      pilotWrite19: process.env.MORSEWORDS_VERIFY_WRITE19 ?? "pending",
  ...
  writeJson(path.join(verificationRoot, "pilot-write-19-verification.json"), report);
  writeMarkdown(report);
  console.log(`Pilot write 19 verification: ${pass} pass, ${warnAccepted} warn accepted, ${fail} fail; raw/generated ${rawExact}/${books.length} exact.`);
