...
const dryRunRoot = path.join(auditRoot, "pilot-dry-run-18");
const writeRoot = path.join(auditRoot, "pilot-write-18");
const verificationRoot = path.join(auditRoot, "pilot-write-18-verification");
...
function writeMarkdown(report: JsonRecord) {
  const lines = [
    "# Pilot write batch 18 verification", "", `Generated: ${report.generatedAt}`, "", "## Summary", "",
  ...
  writeText(path.join(verificationRoot, "pilot-write-18-verification.md"), `${lines.join("\n").trimEnd()}\n`);
}

function main() {
  const requiredReports = [
    path.join(dryRunRoot, "pilot-dry-run-18.json"),
    path.join(dryRunRoot, "pilot-dry-run-18.md"),
    path.join(writeRoot, "pilot-write-18.json"),
    path.join(writeRoot, "pilot-write-18.md"),
  ...
  const dry = readJson(path.join(dryRunRoot, "pilot-dry-run-18.json"));
  const write = readJson(path.join(writeRoot, "pilot-write-18.json"));
  const batch12 = readJson(path.join(auditRoot, "batch-12-prose-restoration/batch-12-prose-restoration.json"));
  if (dry.reportName !== "pilot-dry-run-18" || write.reportName !== "pilot-write-18") throw new Error("Report identity mismatch.");
  ...
  const report = {
    schemaVersion: 1,
    reportName: "pilot-write-18-verification",
    generatedAt: new Date().toISOString(),
    branch: "morsewords-book-processing-pilot-write-18-jun-2026",
    sourceWriteCommit: "9785361f9f5766a36b9d7f1f1d0c644ccb640d95",
    scope: "post-write QA/review of the exact 20 pilot write batch 18 books; no additional books processed",
    ...
    write12SharedScriptScopeFinding: {
      files: ["scripts/books/pilot-book-processing-write-12.ts", "scripts/books/pilot-book-processing-write-18.ts"],
    ...
    specialFocus: {
      exactSpelling: "Tontlawald, Mogarzea, Schippeitaro, Stan Bolovan, and Djulung match source-audited titles exactly.",
      andrewLangEditorRole: "20/20 rights reports preserve Andrew Lang as editor from visible source evidence; no batch-18 title uses Unknown Author.",
    ...
    validationResults: {
      ...
      pilotWrite18: process.env.MORSEWORDS_VERIFY_WRITE18 ?? "pending",
    ...
    backlogNote: write.laterPhaseRequirements ?? [],
    books,
  };
  writeJson(path.join(verificationRoot, "pilot-write-18-verification.json"), report);
  writeMarkdown(report);
  console.log(`Pilot write 18 verification: ${pass} pass, ${warnAccepted} warn accepted, ${fail} fail; raw/generated ${rawExact}/${books.length} exact.`);
