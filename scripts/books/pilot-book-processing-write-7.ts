...
const dryRunRoot = path.join(
  repoRoot,
  "app/client/assets/books/audit-reports/pilot-dry-run-7",
);
const writeReportRoot = path.join(
  repoRoot,
  "app/client/assets/books/audit-reports/pilot-write-7",
);
const dryRunReportPath = path.join(dryRunRoot, "pilot-dry-run-7.json");
...
const EXPECTED_FINAL_SECTION_COUNTS: Record<string, number> = {
  "the-virginian-a-horseman-of-the-plains": 36,
  "the-warden": 21,
};
...
function makeMetadata(
  ...
  return {
    ...
    source: {
      ...
      rightsReviewed: true,
      rightsNotes:
        "Pilot write pass 7 processed this audited Project Gutenberg source for review-gated MorseWords book output.",
    },
    cover: {
      src: null,
      placeholder: true,
...
function buildManifest(
  ...
  return {
    ...
    source: {
      ...
      rightsNotes:
        "Pilot write pass 7 processed this source from the audited Project Gutenberg text. Review generated output before any Cloudflare export.",
    },
    cover: {
      src: null,
      placeholder: true,
    ...
    warnings: [
      "Generated by controlled pilot write pass 7; review before scaling to larger batches or Cloudflare export.",
...
function buildRightsReport(
  ...
  return {
    ...
    reasoning_summary:
      "Controlled pilot write pass 7 used audited Project Gutenberg public-domain source text after dry-run review. Generated output remains review-gated before any Cloudflare export.",
    evidence_snippets: [
...
Processed by pilot write pass 7.
...
function makeSkippedReport(dryRun: DryRunBook, reason: string): BookReport {
  return {
    slug: dryRun.slug,
...
function processSelectedBook(dryRun: DryRunBook): {
  ...
  if (dryRun.currentStatus !== "needs first-time controlled processing" || dryRun.candidateType !== "raw-only") {
    return {
      report: makeSkippedReport(dryRun, "Dry-run entry is not a raw-only first-time processing candidate."),
      manifest: null,
      previewEntry: null,
    };
  }
  ...
  if (!firstDefault || previewLooksUnsafe(firstDefault.text)) {
    return {
      report: makeSkippedReport(dryRun, "First default section did not pass readable-content safety checks."),
      manifest: null,
      previewEntry: null,
    };
  }
  ...
  if (previewLooksUnsafe(preview.asset.previewText)) {
    return {
      report: makeSkippedReport(dryRun, "Generated preview did not pass readable-content safety checks."),
      manifest: null,
      previewEntry: null,
    };
  }
...
function writeMarkdownReport(report: {
  ...
  writeText(path.join(writeReportRoot, "pilot-write-7.md"), `${lines.join("\n").trimEnd()}\n`);
}

function main() {
  const dryRun = readJson<DryRunReport>(dryRunReportPath);
  if (dryRun.reportName !== "pilot-dry-run-7") {
  ...
  writeJson(path.join(writeReportRoot, "pilot-write-7.json"), jsonReport);
  writeMarkdownReport({
    generatedAt,
  ...
  console.log(
    `Pilot write 7 complete: ${processed.length} first-time processed, ${skipped.length} skipped, ${dryRun.unresolvedSourceGeneratedBooksLeftUntouched.length} unresolved-source generated books untouched.`,
  );
}
