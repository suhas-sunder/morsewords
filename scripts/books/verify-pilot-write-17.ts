...
const dryRunRoot = path.join(auditRoot, "pilot-dry-run-17");
const writeRoot = path.join(auditRoot, "pilot-write-17");
const verificationRoot = path.join(auditRoot, "pilot-write-17-verification");
...
const GRIMM = new Set(SELECTED_BATCH.slice(0, 7));
const ANDERSEN = new Set(SELECTED_BATCH.slice(7, 11));
const MIXED = new Set(SELECTED_BATCH.slice(11));
...
// Mirrors the controlled writer's review-gated sanitation so the comparison is
// made from the raw source, not from one generated copy to another.
...
function sourceLooksUnsafe(text: string): boolean {
  return /Project Gutenberg|Gutenberg License|START OF (?:THE|THIS) PROJECT GUTENBERG|END OF (?:THE|THIS) PROJECT GUTENBERG|www\.gutenberg|Distributed Proofreading|Transcriber's Notes?|^Author:|^Title:|^Release date:|^Edited by Andrew Lang$|^By Jacob Grimm and Wilhelm Grimm$|Contact Us|Site Map|All Rights Reserved|(?:^|\n)\s*(?:CONTENTS|TABLE OF CONTENTS)\s*(?:\n|$)/im.test(text);
}
...
function creatorSummary(slug: string): string {
  if (GRIMM.has(slug as (typeof SELECTED_BATCH)[number])) {
    return "Jacob Grimm and Wilhelm Grimm match the raw Author/byline evidence and the existing semicolon-joined Grimm convention.";
  }
  if (ANDERSEN.has(slug as (typeof SELECTED_BATCH)[number])) {
    return "H. C. Andersen remains the source-backed author; J. H. Stickney is retained as editor evidence and does not replace Andersen in the author field.";
  }
  return "Andrew Lang is source-backed as editor; the legacy author array follows the existing tale metadata convention while the write report records the editor role explicitly.";
}

function roleEvidencePass(dryBook: JsonRecord, writeBook: JsonRecord, rawText: string): boolean {
  const evidence = (dryBook.metadataEvidence ?? []) as JsonRecord[];
  const evidencePresent = evidence.every((item) => {
    const evidenceText = String(item.text ?? "");
    if (rawText.includes(evidenceText)) return true;
    return GRIMM.has(dryBook.slug) && item.source === "Gutenberg Author line" &&
      /^Author:\s*Jacob Grimm\s*\r?\n\s*Wilhelm Grimm\s*$/im.test(rawText);
  });
  return evidence.length > 0 &&
    evidencePresent &&
    String(dryBook.expectedCreatorRole ?? "").length > 0 &&
    writeBook.expectedCreatorRole === dryBook.expectedCreatorRole &&
    String(writeBook.generatedCreatorRole ?? "").includes(String(dryBook.expectedCreatorRole));
}
...
function verifyBook(dryBook: JsonRecord, writeBook: JsonRecord) {
  ...
  for (const required of [sourcePath, manifestPath, cleanedPath, processedPath, rightsPath, notesPath, previewPath, perBookDryJsonPath, perBookDryMdPath]) {
  ...
  const evidencePass = roleEvidencePass(dryBook, writeBook, rawText);
  ...
  const checks = {
    ...
    metadata:
      ...
      evidencePass && rights.author === generatedAuthors.join(", "),
    rawGenerated:
    ...
    sectioning:
      dryBook.detectedStructuralConvention === "single contiguous story section" &&
    ...
    cleanup:
      generatedBody === rawBody && parentTitleExcluded && !sourceLooksUnsafe(generatedBody) &&
      !cleanupArtifactsRemain(generatedBody) && !notes.includes("Cloudflare export completed"),
    preview:
      preview.slug === slug && preview.contentHash === manifest.contentHash &&
    ...
    allDefault:
      ...
      defaults[0]?.id === sections[0]?.id && generatedBody.startsWith(String(firstSection.displayText ?? "").slice(0, 120)),
    writeReport:
      writeBook?.finalAction === "first-time processed" && writeBook?.startupPreviewValid === true &&
    ...
    sourceEvidenceLines: rawLines.length > 0 && perBookDryPass,
  };
  ...
  return {
    ...
    titleVerdict: verdict(checks.title ? "pass" : "fail", checks.title
      ? `Individual title preserved exactly as "${manifest.title}"; parent collection "${parentTitle}" is excluded from the title and playback.`
    ...
    authorCompilerCollectorTranslatorRetellerVerdict: verdict(checks.metadata ? "pass" : "fail", checks.metadata
      ? creatorSummary(slug)
      : "Generated creator metadata is missing, unknown, inconsistent with source evidence, or loses the audited creator role.",
      [...(dryBook.metadataEvidence ?? []).map((item: JsonRecord) => `${item.source}: ${item.text}`), `Audited role: ${dryBook.expectedCreatorRole}`]),
    ...
    sectioningVerdict: verdict(checks.sectioning ? "pass" : "fail", checks.sectioning
      ? slug === "sunshine-stories"
        ? "The framed tale has no meaningful internal source headings; one contiguous source-based section preserves its structure without arbitrary flattening."
    ...
    cleanupProsePreservationVerdict: verdict(checks.cleanup ? "pass" : "fail", checks.cleanup
      ? "No parent collection title, TOC, source/license/contributor/transcriber/byline-only material enters playback; prose, dialogue, punctuation, quotes, wrapped lines, and ending remain exact."
    ...
    allMainReadableDefaultVerdict: verdict(checks.allDefault ? "pass" : "fail", checks.allDefault
      ? "All readable story content is included by default and selected/default source order starts at the first section."
...
function writeMarkdown(report: JsonRecord) {
  const lines = [
    "# Pilot write batch 17 verification", "", `Generated: ${report.generatedAt}`, "", "## Summary", "",
    ...
    `- Result: ${report.metadataUnknownAuthorReview.result}`,
    `- Remaining case: ${report.metadataUnknownAuthorReview.slug}`,
    `- Batch 17 introduced it: ${report.metadataUnknownAuthorReview.introducedByBatch17 ? "yes" : "no"}`,
    `- Justification: ${report.metadataUnknownAuthorReview.justification}`, "",
    ...
    "## Special Focus", "",
    `- Grimm group: ${report.specialFocus.grimm}`,
    `- Andersen/literary group: ${report.specialFocus.andersen}`,
    `- Mixed children's/fairy-tale group: ${report.specialFocus.mixed}`, "",
    "## Books", "",
    ...
    `- Playwright known fullscreen-only failure: ${report.playwright.knownFullscreenOnlyFailure ? "yes" : "no"}`,
    `- Audit side-effect handling: ${report.auditSideEffectHandling.result}`, "",
  ];
  writeText(path.join(verificationRoot, "pilot-write-17-verification.md"), `${lines.join("\n").trimEnd()}\n`);
}

function main() {
  const requiredReports = [
    path.join(dryRunRoot, "pilot-dry-run-17.json"),
    path.join(dryRunRoot, "pilot-dry-run-17.md"),
    path.join(writeRoot, "pilot-write-17.json"),
    path.join(writeRoot, "pilot-write-17.md"),
  ...
  const dry = readJson(path.join(dryRunRoot, "pilot-dry-run-17.json"));
  const write = readJson(path.join(writeRoot, "pilot-write-17.json"));
  const batch12 = readJson(path.join(auditRoot, "batch-12-prose-restoration/batch-12-prose-restoration.json"));
  const metadataAudit = readJson(path.join(auditRoot, "metadata-segmentation-correctness-audit-1/metadata-segmentation-correctness-audit-1.json"));
  if (dry.reportName !== "pilot-dry-run-17" || write.reportName !== "pilot-write-17") throw new Error("Report identity mismatch.");
  ...
  const unknownCases = (metadataAudit.books ?? []).filter((book: JsonRecord) => book.generatedAuthorIsUnknown);
  const unknownCase = unknownCases.find((book: JsonRecord) => book.slug === "the-arabian-nights");
  ...
  const report = {
    schemaVersion: 1,
    reportName: "pilot-write-17-verification",
    generatedAt: new Date().toISOString(),
    branch: "morsewords-book-processing-pilot-write-17-jun-2026",
    scope: "post-write QA/review of the exact 20 pilot write batch 17 books",
    ...
    write12SharedScriptScopeFinding: {
      files: ["scripts/books/pilot-book-processing-write-12.ts", "scripts/books/pilot-book-processing-write-17.ts"],
    ...
    metadataUnknownAuthorReview: {
      result: unknownCases.length === 1 && unknownCase?.unknownAuthorJustified === true ? "pass" : "fail",
      slug: unknownCase?.slug ?? null,
      introducedByBatch17: SELECTED_BATCH.includes(unknownCase?.slug),
      source: unknownCase?.rawSourceFileUsed ?? null,
      justification: unknownCase?.warnings?.join("; ") ?? "Expected documented unknown-author case was not found.",
    },
    ...
    specialFocus: {
      grimm: "7/7 use individual tale titles, source-backed Grimm metadata, true prose starts/endings, and no parent collection or TOC playback.",
      andersen: "4/4 preserve H. C. Andersen as author and J. H. Stickney as editor evidence. Sunshine Stories is one source-based framed tale with no meaningful internal headings to split.",
      mixed: "9/9 preserve exact title spelling/casing, Andrew Lang's editor role evidence, true first/last readable lines, and wrapped prose character-for-character.",
    },
    ...
    validationResults: {
      ...
      pilotWrite17: process.env.MORSEWORDS_VERIFY_WRITE17 ?? "pending",
      ...
      browserQa: process.env.MORSEWORDS_VERIFY_BROWSER ?? "pending",
  ...
  writeJson(path.join(verificationRoot, "pilot-write-17-verification.json"), report);
  writeMarkdown(report);
  console.log(`Pilot write 17 verification: ${pass} pass, ${warnAccepted} warn accepted, ${fail} fail; raw/generated ${rawExact}/${books.length} exact.`);
  if (fail || !batch12Pass || report.metadataUnknownAuthorReview.result !== "pass") process.exitCode = 1;
}
