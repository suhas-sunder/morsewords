...
const reportRoot = path.join(auditRoot, "pilot-dry-run-9");
const reportBooksRoot = path.join(reportRoot, "books");
const mainJsonPath = path.join(reportRoot, "pilot-dry-run-9.json");
const mainMarkdownPath = path.join(reportRoot, "pilot-dry-run-9.md");
...
const titleOverrides: Record<string, string> = {
  ...
  "the-three-taps-a-detective-story-without-a-moral": "The Three Taps",
};
...
function buildReport() {
  ...
  const allowedBranches = new Set([
    "main",
    "morsewords-book-processing-pilot-dry-run-9-jun-2026",
  ]);
  if (!allowedBranches.has(branch)) {
    throw new Error(`Expected dry-run 9 validation branch, got ${branch}`);
  }
  ...
  console.log(`Pilot dry-run 9 selected: ${report.selectedCount}`);
  console.log(
