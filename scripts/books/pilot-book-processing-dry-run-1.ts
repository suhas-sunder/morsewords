...
type Pass2Report = {
  books: Pass2Book[];
  recommendedPilotBatch: Array<{ slug: string }>;
};
...
const DRY_RUN_ROOT = path.join(AUDIT_REPORT_ROOT, "pilot-dry-run-1");
...
const MAIN_JSON_PATH = path.join(DRY_RUN_ROOT, "pilot-dry-run-1.json");
const MAIN_MARKDOWN_PATH = path.join(DRY_RUN_ROOT, "pilot-dry-run-1.md");
...
function finalRecommendation(book: Pass2Book, comparison: ExistingGeneratedComparison): {
...
function dryRunBook(book: Pass2Book): BookDryRun {
  ...
  const rawText = normalizeBookText(fs.readFileSync(sourcePath, "utf8"));
  const lines = buildLines(rawText);
  const generated = readGeneratedManifest(book.slug);
  const author = extractAuthor(rawText, generated.manifest);
  ...
  const endIndex = lineEndIndex(lines, refinedEndLine) ?? rawText.length;
  const safeStartIndex = Math.max(0, Math.min(rawText.length, startIndex));
  const safeEndIndex = Math.max(safeStartIndex, Math.min(rawText.length, endIndex));
  const frontMatterText = rawText.slice(0, safeStartIndex);
  const keptText = trimBookText(rawText.slice(safeStartIndex, safeEndIndex));
  const endMatterText = rawText.slice(safeEndIndex);
  ...
  const recommendation = finalRecommendation(book, comparison);
  ...
  const result: BookDryRun = {
    ...
    rawWordCount: countBookWords(rawText),
...
function mainMarkdown(report: DryRunReport): string {
  const recommendationRows = report.books
    .map(
      (book) =>
        `| ${book.slug} | ${book.pass2RiskLevel} | ${book.proposedSections.length} | ${book.keptWordCountEstimate} | ${book.finalDryRunRecommendation} | ${book.firstHourPreviewCandidate.feasible ? "yes" : "no"} |`,
    )
    .join("\n");
...
function buildReport(books: BookDryRun[]): DryRunReport {
  ...
  const seemsSafe =
    counts.safeToProcessLater + counts.processLaterWithWarnings >= 7 &&
    counts.blocked === 0 &&
    counts.needsIndividualReview <= 3;
  ...
  return {
    ...
    processorSafetyAssessment: {
      ...
      reason: seemsSafe
        ? "The dry run produced reviewable outputs for all 10 pilot books with no blocked sources and enough safe/warning candidates for a controlled write pass."
        : "Several pilot books still require individual review, so a real write pass should wait until those are reviewed or excluded.",
      recommendedNextStep: seemsSafe
        ? "Run a real pilot write pass only for the safe/warning subset first, keeping high-risk books in individual review."
        : "Review high-risk per-book reports, then run a smaller real write pass for safe and warning-only books.",
    },
    confirmations: {
      tempBooksModified: false,
      generatedOutputsModified: false,
...
function main(): void {
  ...
  const pass2Pilot = pass2.recommendedPilotBatch.map((entry) => entry.slug);
  const missingFromPass2Pilot = PILOT_BATCH.filter((slug) => !pass2Pilot.includes(slug));
  if (missingFromPass2Pilot.length > 0) {
    throw new Error(`Requested pilot slugs are missing from pass-2 pilot batch: ${missingFromPass2Pilot.join(", ")}`);
  }
  ...
  console.log("Pilot dry run 1 complete.");
