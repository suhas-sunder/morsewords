...
type SkippedBookVerification = {
  slug: string;
  writeAction: "skipped" | null;
  skipped: boolean;
  noGeneratedOutputCreated: boolean;
  noPreviewAssetCreated: boolean;
  duplicateOrBoundaryRiskConfirmed: boolean;
  existingGeneratedTarget: string | null;
  existingGeneratedTargetUnchanged: boolean | null;
  skipReason: string | null;
  acceptedForSkip: boolean;
  acceptedAsNewlyProcessed: boolean;
};
...
const dryRunRoot = path.join(auditRoot, "pilot-dry-run-9");
const writeRoot = path.join(auditRoot, "pilot-write-9");
const verificationRoot = path.join(auditRoot, "pilot-write-9-verification");

const SKIPPED_BATCH = [
  {
    slug: "the-two-magics-the-turn-of-the-screw-covering-end",
    riskType: "duplicate",
    expectedReasonFragments: [
      "contains a full The Turn of the Screw",
      "generated the-turn-of-the-screw already exists",
      "no distinct-version policy",
    ],
    existingGeneratedTarget: "the-turn-of-the-screw",
  },
  {
    slug: "the-works-of-edgar-allan-poe",
    riskType: "boundary",
    expectedReasonFragments: [
      "raw Volume 2 begins with THE PURLOINED LETTER",
      "dry-run proposed starting at the next story",
      "drop real opening collection content",
    ],
    existingGeneratedTarget: null,
  },
] as const;
...
function verifyProcessedBook(
  ...
  return {
    ...
    dryRunReportInspected: repoPath(path.join(dryRunRoot, "pilot-dry-run-9.json")),
    dryRunPerBookReportsInspected: [repoPath(dryRunBookJsonPath), repoPath(dryRunBookMdPath)],
    writeReportInspected: repoPath(path.join(writeRoot, "pilot-write-9.json")),
    generatedTitleVerdict,
    generatedAuthorVerdict,
...
function writeMarkdownReport(report: {
  ...
  skippedBooks: SkippedBookVerification[];
  ...
  const lines = [
    ...
    "## Skipped books",
    "",
    ...report.skippedBooks.flatMap((book) => [
      `### ${book.slug}`,
      "",
      `- Skipped: ${book.skipped ? "yes" : "no"}`,
      `- No generated output created: ${book.noGeneratedOutputCreated ? "yes" : "no"}`,
      `- No preview asset created: ${book.noPreviewAssetCreated ? "yes" : "no"}`,
      `- Duplicate/boundary risk confirmed: ${book.duplicateOrBoundaryRiskConfirmed ? "yes" : "no"}`,
      `- Existing generated target: ${book.existingGeneratedTarget ?? "n/a"}`,
      `- Existing generated target unchanged: ${
        book.existingGeneratedTargetUnchanged === null
          ? "n/a"
          : book.existingGeneratedTargetUnchanged
            ? "yes"
            : "no"
      }`,
      `- Accepted for skip: ${book.acceptedForSkip ? "yes" : "no"}`,
      `- Accepted as newly processed: ${book.acceptedAsNewlyProcessed ? "yes" : "no"}`,
      `- Skip reason: ${book.skipReason ?? "n/a"}`,
      "",
    ]),
  ...
  writeText(
    path.join(verificationRoot, "pilot-write-9-verification.md"),
    `${lines.join("\n").trimEnd()}\n`,
  );
}

function main() {
  const dryRunJsonPath = path.join(dryRunRoot, "pilot-dry-run-9.json");
  const dryRunMdPath = path.join(dryRunRoot, "pilot-dry-run-9.md");
  const writeJsonPath = path.join(writeRoot, "pilot-write-9.json");
  const writeMdPath = path.join(writeRoot, "pilot-write-9.md");
  ...
  if (dryRun.reportName !== "pilot-dry-run-9" || write.reportName !== "pilot-write-9") {
    throw new Error("Missing or incorrect batch-9 dry-run/write reports.");
  }
  if (JSON.stringify(dryRun.selectedBooks) !== JSON.stringify([...SELECTED_BATCH])) {
    throw new Error("Dry-run selectedBooks do not match the expected batch-9 list.");
  }
  if (JSON.stringify(write.selectedBooks) !== JSON.stringify([...SELECTED_BATCH])) {
    throw new Error("Write selectedBooks do not match the expected batch-9 list.");
  }
  if (
    dryRun.selectedCount !== 20 ||
    write.totals.selected !== 20 ||
    write.totals.firstTimeProcessed !== 18 ||
    write.totals.skipped !== 2
  ) {
    throw new Error("Write report is missing or incomplete for batch 9.");
  }
  ...
  const skippedBooks: SkippedBookVerification[] = SKIPPED_BATCH.map((skip) => {
    const skippedWrite = write.books.find((book) => book.slug === skip.slug);
    const skippedGeneratedPath = path.join(generatedRoot, skip.slug);
    const skippedPreviewPath = path.join(previewRoot, `${skip.slug}.preview.json`);
    const existingGeneratedPath = skip.existingGeneratedTarget
      ? path.join(generatedRoot, skip.existingGeneratedTarget)
      : null;
    const existingPreviewPath = skip.existingGeneratedTarget
      ? path.join(previewRoot, `${skip.existingGeneratedTarget}.preview.json`)
      : null;
    const existingTargetGitStatus =
      existingGeneratedPath && existingPreviewPath
        ? gitStatusFor([repoPath(existingGeneratedPath), repoPath(existingPreviewPath)])
        : [];
    const skipReason =
      skippedWrite?.duplicateNearDuplicateSlugCheckResult ??
      skippedWrite?.remainingWarnings.join("; ") ??
      null;
    const duplicateOrBoundaryRiskConfirmed =
      skippedWrite?.finalAction === "skipped" &&
      typeof skipReason === "string" &&
      skip.expectedReasonFragments.every((fragment) => skipReason.includes(fragment));

    return {
      slug: skip.slug,
      writeAction: skippedWrite?.finalAction === "skipped" ? "skipped" : null,
      skipped: skippedWrite?.finalAction === "skipped",
      noGeneratedOutputCreated: !fs.existsSync(skippedGeneratedPath),
      noPreviewAssetCreated: !fs.existsSync(skippedPreviewPath),
      duplicateOrBoundaryRiskConfirmed,
      existingGeneratedTarget: skip.existingGeneratedTarget,
      existingGeneratedTargetUnchanged:
        skip.existingGeneratedTarget === null ? null : existingTargetGitStatus.length === 0,
      skipReason,
  ...
  if (
    skippedBooks.some(
      (book) =>
        !book.skipped ||
        !book.noGeneratedOutputCreated ||
        !book.noPreviewAssetCreated ||
        !book.duplicateOrBoundaryRiskConfirmed ||
        book.existingGeneratedTargetUnchanged === false,
    )
  ) {
    throw new Error("Batch-9 skipped-book verification failed.");
  }
  ...
  const report = {
    schemaVersion: 1,
    reportName: "pilot-write-9-verification",
    generatedAt,
    ...
    totals: {
      ...
      correctionsApplied,
      skippedVerified: skippedBooks.length,
  ...
  writeJson(path.join(verificationRoot, "pilot-write-9-verification.json"), report);
  writeMarkdownReport({
    generatedAt,
  ...
  console.log(
    `Pilot write 9 verification complete: ${pass} pass, ${warnAccepted} warn accepted, ${fail} fail; ${correctionsApplied} corrections documented.`,
  );
