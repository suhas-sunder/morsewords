...
const reportRoot = path.join(auditRoot, "pilot-dry-run-7");
const reportBooksRoot = path.join(reportRoot, "books");
const mainJsonPath = path.join(reportRoot, "pilot-dry-run-7.json");
const mainMarkdownPath = path.join(reportRoot, "pilot-dry-run-7.md");
...
function authorList(value: string | string[]) {
...
function deriveAcceptedSlugs() {
  ...
  for (const relativePath of acceptedReportPaths) {
    ...
    if (relativePath.includes("pilot-write-6-verification")) {
      for (const book of books) {
...
function inspectBook(
  ...
  const authorFromEvidence = evidenceValue(authorEvidence, "Author");
  ...
  const firstPreview = compactText(first.nextProsePreview, 260);
  const sectionExamples = analysis.examplesOfDetectedBodyHeadings
    .slice(0, 6)
    .map((example) => compactText(example, 180));
  ...
  return {
    ...
    detectedStructuralConvention: analysis.detectedStructuralConvention,
    structureConfidence: analysis.confidenceLevel,
    meaningfulHeadingsExist: analysis.selectedBodyHeadings.length > 0,
    ...
    expectedDefaultReadableSections: `all ${analysis.estimatedSectionCount} detected ${analysis.detectedStructuralConvention} sections unless a future write inspection demotes true front/back matter`,
    likelySectionCount: analysis.estimatedSectionCount,
    ...
    expectedSectioningStrategy: `use ${analysis.selectedHeadingStrategy?.label ?? analysis.detectedStructuralConvention} boundaries; never replace meaningful headings with vague Part 1 / Part 2 chunks`,
    cleanupRisks,
...
function buildReport() {
  ...
  if (branch !== "morsewords-book-processing-pilot-dry-run-7-jun-2026") {
    throw new Error(`Expected dry-run 7 branch, got ${branch}`);
  }
  ...
  console.log(`Pilot dry-run 7 selected: ${report.selectedCount}`);
  console.log(
