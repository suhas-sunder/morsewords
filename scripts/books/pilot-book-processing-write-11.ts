...
const dryRunRoot = path.join(
  repoRoot,
  "app/client/assets/books/audit-reports/pilot-dry-run-11",
);
const writeReportRoot = path.join(
  repoRoot,
  "app/client/assets/books/audit-reports/pilot-write-11",
);
const dryRunReportPath = path.join(dryRunRoot, "pilot-dry-run-11.json");
...
const EXPECTED_FINAL_SECTION_COUNTS: Record<string, number> = {
  ...
  "the-ugly-duckling": 1,
  "the-adventures-of-chanticleer-and-partlet": 3,
  "jorinda-and-jorindel": 1,
  "mother-holle": 1,
  "rapunzel": 1,
  "the-juniper-tree": 1,
  "the-seven-ravens": 1,
  "the-wedding-of-mrs-fox": 1,
  "the-adventures-of-kintaro-the-golden-boy": 1,
  "the-bamboo-cutter-and-the-moon-child": 1,
  "the-goblin-of-adachigahara": 1,
...
function makeMetadata(
  ...
  return {
    ...
    source: {
      ...
      rightsReviewed: true,
      rightsNotes:
        "Pilot write pass 11 processed this audited Project Gutenberg source for review-gated MorseWords book output.",
    },
    cover: {
      src: null,
      placeholder: true,
...
function buildManifest(
  ...
  return {
    ...
    source: {
      ...
      rightsNotes:
        "Pilot write pass 11 processed this source from the audited Project Gutenberg text. Review generated output before any Cloudflare export.",
    },
    cover: {
      src: null,
      placeholder: true,
    ...
    warnings: [
      "Generated by controlled pilot write pass 11; review before scaling to larger batches or Cloudflare export.",
...
function buildRightsReport(
  ...
  return {
    ...
    reasoning_summary:
      "Controlled pilot write pass 11 used audited Project Gutenberg public-domain source text after dry-run review. Generated output remains review-gated before any Cloudflare export.",
    evidence_snippets: [
...
function duplicateNearDuplicateCheck(dryRun: DryRunBook): {
  ...
  if (duplicate.slug === dryRun.slug) {
    return {
      blocking: false,
      message: `passed: ${duplicate.slug} is the selected batch-11 slug and may be regenerated by this targeted command`,
    };
  }

  return {
    blocking: true,
    message: `blocked: existing generated slug ${duplicate.slug} already has title "${duplicate.title}" and author ${duplicate.author.join(", ")}; dry-run 11 did not document a distinct-version reason for creating ${dryRun.slug}`,
  };
}
...
Processed by pilot write pass 11.
...
function writeMarkdownReport(report: {
  ...
  writeText(path.join(writeReportRoot, "pilot-write-11.md"), `${lines.join("\n").trimEnd()}\n`);
}

function main() {
  const dryRun = readJson<DryRunReport>(dryRunReportPath);
  if (dryRun.reportName !== "pilot-dry-run-11") {
  ...
  const acceptedExclusionClarification = {
    ...
    note:
      "This write pass uses only the exact selected list from pilot-dry-run-11.json. Known duplicate/manual/boundary exclusions and unresolved-source generated books remain untouched.",
  };
  const jsonReport = {
    schemaVersion: 1,
    reportName: "pilot-write-11",
    generatedAt,
  ...
  writeJson(path.join(writeReportRoot, "pilot-write-11.json"), jsonReport);
  writeMarkdownReport({
    generatedAt,
  ...
  console.log(
    `Pilot write 11 complete: ${processed.length} first-time processed, ${skipped.length} skipped, ${dryRun.unresolvedSourceGeneratedBooksLeftUntouched.length} unresolved-source generated books untouched.`,
  );
}
